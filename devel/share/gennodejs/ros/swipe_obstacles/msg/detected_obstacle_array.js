...
let detected_obstacle = require('./detected_obstacle.js');
let std_msgs = _finder('std_msgs');

//-----------------------------------------------------------

class detected_obstacle_array {
  constructor(initObj={}) {
    if (initObj === null) {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.obstacles.forEach((val) => {
      bufferOffset = detected_obstacle.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    let len;
    let data = new detected_obstacle_array(null);
    ...
    for (let i = 0; i < len; ++i) {
      data.obstacles[i] = detected_obstacle.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    ...
    object.obstacles.forEach((val) => {
      length += detected_obstacle.getMessageSize(val);
    });
    return length + 4;
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new detected_obstacle_array(null);
    ...
    if (msg.obstacles !== undefined) {
      ...
      for (let i = 0; i < resolved.obstacles.length; ++i) {
        resolved.obstacles[i] = detected_obstacle.Resolve(msg.obstacles[i]);
      }
    }
    else {
      resolved.obstacles = []
...
module.exports = detected_obstacle_array;
